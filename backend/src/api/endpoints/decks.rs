...
pub async fn get_categories(
    ...
    match state.deck_use_cases.list_categories_with_counts().await {
        Ok(categories) => (StatusCode::OK, Json(serde_json::json!({ "success": true, "categories": categories }))).into_response(),
...
pub async fn get_deck_data(
    ...
    match state.deck_use_cases.get_deck_data(&query.user_id, &query.category, &query.deck).await {
...
pub async fn reset_all_statuses(
    ...
    if !payload.confirm {
        return Ok((StatusCode::BAD_REQUEST, Json(serde_json::json!({ "success": false, "detail": "Confirmación requerida" }))).into_response());
    }
