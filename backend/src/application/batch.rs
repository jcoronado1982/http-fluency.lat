...
pub async fn run_batch_image_generation(state: AppState, filter: BatchFilter) -> anyhow::Result<()> {
...
async fn run_batch(state: AppState, mode: BatchMode, filter: BatchFilter) -> anyhow::Result<()> {
    ...
    println!("🔍 Categorías a procesar: {} → {:?}", categories.len(), categories);
    let _ = stdout().flush();
    ...
    for cat_name in categories {
        ...
        for deck_name in decks {
            ...
            let mut deck_data = state.deck_use_cases.get_deck_json(&cat_name, &deck_name).await?;
...
fn collect_def_slots(card: &Flashcard) -> Vec<DefSlot> {
    ...
    for (form, irregular_key) in [(FormVariant::V2, "past"), (FormVariant::V3, "participle")] {
            ...
            .and_then(|v| v.get(irregular_key))
        {
            ...
            } else if block.get("usage_example").and_then(|v| v.as_str()).is_some() {
...
fn get_definition_object<'a>(
    ...
    match form {
        ...
        FormVariant::V2 | FormVariant::V3 => {
            let key = if form == FormVariant::V2 { "past" } else { "participle" };
...
fn clear_definition_image_path(
    card: &mut Flashcard,
    def_index: usize,
    form: FormVariant,
) -> bool {
...
mod tests {
    ...
    fn collect_def_slots_includes_v1_v2_v3_when_applicable() {
        ...
        assert_eq!(slots[0], DefSlot { form: FormVariant::V1, def_index: 0 });
        assert_eq!(slots[2], DefSlot { form: FormVariant::V2, def_index: 0 });
        assert_eq!(slots[4], DefSlot { form: FormVariant::V3, def_index: 0 });
    }
    ...
    fn collect_def_slots_includes_irregular_without_definitions_array() {
        ...
        assert_eq!(slots[0], DefSlot { form: FormVariant::V1, def_index: 0 });
        assert_eq!(slots[1], DefSlot { form: FormVariant::V2, def_index: 0 });
        assert_eq!(slots[2], DefSlot { form: FormVariant::V3, def_index: 0 });
    }
    ...
    fn set_and_get_image_path_for_v2() {
        ...
        assert!(set_definition_image_path(&mut card, 1, path, FormVariant::V2));
        assert_eq!(
    ...
    fn set_and_get_image_path_for_v2_flat() {
        ...
        assert!(set_definition_image_path(&mut card, 0, path, FormVariant::V2));
        assert_eq!(
