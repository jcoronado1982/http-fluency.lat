...
//!     cargo run --manifest-path backend/Cargo.toml -- --batch-gen-audio [categoría] [deck]
//!
//! Ejemplos:
...
pub async fn run_batch_audio_generation(state: AppState, filter: BatchFilter) -> anyhow::Result<()> {
...
async fn run_batch_audio(state: AppState, filter: BatchFilter) -> anyhow::Result<()> {
    ...
    let run_phonics = filter.category.as_deref().map(|c| c == PHONICS_CATEGORY).unwrap_or(true)
        && filter.deck.is_none();
    ...
    for cat_name in categories {
        ...
        for deck_name in decks {
            ...
            let deck_data = state.deck_use_cases.get_deck_json(&cat_name, &deck_name).await?;
...
async fn process_phonics(
    ...
    for (ri, rule) in rules.iter().enumerate() {
        ...
        for example in examples {
            ...
            let req = build_en_audio_request(PHONICS_CATEGORY, PHONICS_DECK, text.clone(), Some(text.clone()));
...
fn print_summary(global_counter: usize, stats: &BatchAudioStats, failures_log: &str) {
    ...
    if stats.failures.is_empty() {
        ...
        println!("\n📋 DETALLE — AUDIOS NO CREADOS ({})", stats.failures.len());
...
fn extract_usage_example_en(card: &Flashcard, def_index: usize, form: FormVariant) -> Option<String> {
