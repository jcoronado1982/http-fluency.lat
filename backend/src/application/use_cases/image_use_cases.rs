...
impl ImageUseCases {
    ...
    pub async fn get_or_generate_image(&self, req: &ImageGenRequest, user_email: &str, role: &str) -> Result<(String, bool)> {
        tracing::info!(
            "🖼️ Solicitud de imagen: email='{}', rol='{}', deck='{}', index='{}', prompt='{}'",
            user_email, role, req.deck, req.index, req.prompt
        );
        ...
        if !is_admin && !is_premium {
            ...
            return Err(anyhow::anyhow!("No autorizado para generar imágenes por IA (requiere plan Premium)"));
        }
        ...
        let base_pattern = match &user_segment {
            Some(seg) => format!("users/{}/{}/{}/{}_card_{}_def{}{}", seg, req.category, deck_prefix, deck_prefix, req.index, req.def_index, form_suffix),
            None      => format!("{}/{}/{}_card_{}_def{}{}", req.category, deck_prefix, deck_prefix, req.index, req.def_index, form_suffix),
        };
        ...
        if !req.force_generation {
            ...
            if let Ok(true) = self.storage_repo.blob_exists(&avif_path).await {
                return Ok((format!("/card_images/{}.avif?v={}", base_pattern, uuid::Uuid::new_v4()), false));
            }

            // Para usuarios normales: fallback a capa global antes de generar
            if !is_admin {
                let global_base = format!("{}/{}/{}_card_{}_def{}{}", req.category, deck_prefix, deck_prefix, req.index, req.def_index, form_suffix);
                let global_avif = format!("{}/{}.avif", self.settings.gcs_images_prefix, global_base);
                if let Ok(true) = self.storage_repo.blob_exists(&global_avif).await {
                    return Ok((format!("/card_images/{}.avif?v={}", global_base, uuid::Uuid::new_v4()), false));
                }
            }
            ...
            if !form_suffix.is_empty() {
                if let Some(seg) = &user_segment {
                    ...
                    let v1_personal_avif = format!("{}/{}.avif", self.settings.gcs_images_prefix, v1_personal);
                    if let Ok(true) = self.storage_repo.blob_exists(&v1_personal_avif).await {
                        tracing::info!("↩️ v2/v3 sin imagen propia, reutilizando v1 personal: {}", v1_personal_avif);
                        return Ok((format!("/card_images/{}.avif?v={}", v1_personal, uuid::Uuid::new_v4()), false));
                    }
                }
                ...
                let v1_global_avif = format!("{}/{}.avif", self.settings.gcs_images_prefix, v1_global);
                if let Ok(true) = self.storage_repo.blob_exists(&v1_global_avif).await {
                    tracing::info!("↩️ v2/v3 sin imagen propia, reutilizando v1 global: {}", v1_global_avif);
                    return Ok((format!("/card_images/{}.avif?v={}", v1_global, uuid::Uuid::new_v4()), false));
                }
            }
        }

        if self.settings.gemini_api_key.is_none() {
            return Err(anyhow::anyhow!("La generación de imagen por IA está deshabilitada."));
        }
        ...
        Ok((format!("/card_images/{}?v={}", file_name, uuid::Uuid::new_v4()), true))
    }
    ...
    pub async fn delete_image(
        ...
        let base_pattern = if is_admin {
            format!("{}/{}/{}_card_{}_def{}{}", category, deck_prefix, deck_prefix, index, def_index, form_suffix)
        } else {
            let seg = user_path_segment(user_email);
            format!("users/{}/{}/{}/{}_card_{}_def{}{}", seg, category, deck_prefix, deck_prefix, index, def_index, form_suffix)
        };

        let mut deleted_any = false;
        for ext in [".avif", ".jpg", ".png", ".jpeg"] {
            let blob_path = format!("{}/{}{}", self.settings.gcs_images_prefix, base_pattern, ext);
    ...
    pub async fn resolve_image_path(
        ...
        tracing::info!(
            ...
            category, deck, index, def_index, form, role
        );
        ...
        tracing::info!("  ❌ no image found for {}/{}_card_{}_def{}{}", category, deck_prefix, index, def_index, form_suffix);
        Ok(None)
    }
    ...
    pub async fn upload_image(&self, req: UploadImageRequest, user_email: &str, is_admin: bool) -> Result<String> {
        let deck_prefix = req.deck.replace(".json", "");
        let form_suffix = self.form_suffix(req.form.as_deref());
        ...
        let final_name = if is_admin {
            format!("{}/{}/{}_card_{}_def{}{}.{}", req.category, deck_prefix, deck_prefix, req.card_index, req.def_index, form_suffix, extension)
        } else {
            let seg = user_path_segment(user_email);
            format!("users/{}/{}/{}/{}_card_{}_def{}{}.{}", seg, req.category, deck_prefix, deck_prefix, req.card_index, req.def_index, form_suffix, extension)
        };
        let blob_path = format!("{}/{}", self.settings.gcs_images_prefix, final_name);

        tracing::info!("📤 Uploading manual image to: {}", blob_path);
        self.storage_repo.upload_blob(&blob_path, req.file_data, &req.content_type).await?;

        Ok(format!("/card_images/{}?v={}", final_name, uuid::Uuid::new_v4()))
    }
