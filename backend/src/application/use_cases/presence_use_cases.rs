...
use crate::domain::models::user_activity::{AdminUserActivity, PaginatedAdminUsers, ClientInfo};
...
impl PresenceUseCases {
    ...
    pub async fn get_admin_dashboard(&self, page: usize, limit: usize) -> Result<PaginatedAdminUsers> {
    ...
    async fn close_session(&self, email: &str) {
        if let Some((_, session)) = self.active.remove(email) {
            let duration = (session.last_seen - session.session_start).num_seconds().max(0);
            let _ = self
                .activity_repo
