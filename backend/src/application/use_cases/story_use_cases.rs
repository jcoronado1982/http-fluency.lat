...
impl StoryUseCases {
    ...
    pub async fn get_progress(&self, user_id: &str, story_id: i32) -> Result<ProgressResponse> {
        let progress = match self.db_repo.get_progress(user_id, story_id).await? {
            ...
            None => {
                ...
                self.db_repo.create_progress(user_id, story_id, first_ep_id).await?
            }
        };

        // Proactive image generation in background
        ...
        let episode_title = self.db_repo.get_episode_title(progress.current_episode_id).await?;
    ...
    async fn prefetch_images_internal(
        ...
        info!("Starting proactive image generation for all episodes of story {}", story_id);
        ...
        for (ep_id, ep_title) in episodes {
            ...
            if screens_to_gen.is_empty() {
                info!("All images for story {} / ep {} are already generated.", story_id, ep_id);
                continue;
            }
            ...
            let prompts_list =
                ai_tutor.improve_visual_prompts_batch(&json!(story_data_for_ai), &ep_title).await?;

            for (i, screen) in screens_to_gen.iter().enumerate() {
                let visual_prompt = prompts_list
                    .get(i)
                    .cloned()
                    .or_else(|| screen.content["ai_visual_prompt"].as_str().map(str::to_string))
                ...
                match image_gen.generate(&visual_prompt).await {
                    Ok(raw_bytes) => {
                        ...
                        db_repo.update_screen_content(screen.id, new_content).await?;
