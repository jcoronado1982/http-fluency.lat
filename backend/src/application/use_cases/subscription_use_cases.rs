...
impl SubscriptionUseCases {
    pub fn new(repository: Arc<dyn SubscriptionRepository>, payment: Arc<dyn PaymentProvider>) -> Self {
    ...
    pub async fn activate(&self, email: &str, plan: SubscriptionPlan) -> Result<Subscription> {
        ...
        let (payment_provider, external_customer_id, external_subscription_id) =
            if self.payment.name() != "null" {
                let customer_id = match existing.as_ref().and_then(|s| s.external_customer_id.clone()) {
                    Some(id) => id,
                    None => self.payment.create_customer(email).await?,
                };
                let payment_ref = self.payment.create_subscription(&customer_id, &plan).await?;
                (
                ...
                (
                    ...
                    existing.as_ref().and_then(|s| s.external_customer_id.clone()),
                    existing.as_ref().and_then(|s| s.external_subscription_id.clone()),
                )
            };
    ...
    pub async fn list_all(&self, limit: usize, offset: usize) -> Result<Vec<Subscription>> {
        self.repository.list_subscriptions(limit.min(100), offset).await
    }

    /// Genera una URL de checkout para que el usuario pague directamente.
    /// Solo disponible cuando el proveedor no es `null`.
    pub async fn checkout_url(&self, email: &str, plan: SubscriptionPlan, return_url: &str) -> Result<String> {
        if self.payment.name() == "null" {
            return Err(anyhow!("No hay proveedor de pago configurado. Contacta al administrador."));
        }
        ...
        self.payment.get_checkout_url(&customer_id, &plan, return_url).await
    }
