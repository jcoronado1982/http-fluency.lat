...
impl TutorUseCases {
    pub fn new(ai_tutor: Arc<dyn AITutor>, db_repo: Arc<dyn StoryArcadeRepository>) -> Self {
        Self { ai_tutor, db_repo }
    }

    pub async fn analyze_error(&self, request: TutorRequest) -> Result<String> {
        let result_str = self.ai_tutor.analyze_error(&request.user_input, &request.correct_answer, &request.context_spanish).await?;
        ...
        if let Ok(data) = serde_json::from_str::<Value>(&result_str) {
            match (request.user_id, request.story_id, request.screen_id) {
                (Some(user_id), Some(story_id), Some(screen_id)) 
                    if !data["is_correct"].as_bool().unwrap_or(true) => {
                    
                    let _ = self.db_repo.log_user_error(
                        &user_id, 
                        story_id, 
                        screen_id, 
                        &request.user_input, 
    ...
    pub async fn explain_like_child(&self, request: TutorRequest) -> Result<String> {
        self.ai_tutor.explain_like_child(
            &request.user_input, 
            &request.correct_answer, 
