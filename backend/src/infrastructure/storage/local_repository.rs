...
impl LocalStorageRepository {
    ...
    fn control_path(&self) -> String {
        format!("/tmp/oracle-cm-{}", self.oracle_host.replace('.', "-").replace(':', "-"))
    }
    ...
    async fn fetch_from_oracle(&self, path: &str) -> Result<Vec<u8>> {
        let url = format!("{}/{}", self.oracle_base_url(), path.trim_start_matches('/'));
        let res = self.http_client.get(&url).send().await
            .context(format!("HTTP GET fallido: {}", url))?;
        if res.status().is_success() {
            Ok(res.bytes().await?.to_vec())
        } else {
            Err(anyhow::anyhow!("Oracle devolvió {} para {}", res.status(), url))
        }
    }
    ...
    async fn list_oracle_entries(&self, rel_path: &str, dirs_only: bool) -> Result<Vec<String>> {
        let url = format!("{}/{}/", self.oracle_base_url(), rel_path.trim_start_matches('/'));
        let res = self.http_client.get(&url).send().await
    ...
    async fn scp_to_oracle(&self, local_path: &Path, remote_relative: &str) -> Result<()> {
        ...
        let remote_target = format!("root@{}:\"{}\"", self.oracle_host, remote_file.replace("\"", "\\\""));
        ...
        if !mkdir.success() {
            return Err(anyhow::anyhow!(
                "ssh mkdir -p '{}' falló con código {:?}", remote_dir, mkdir.code()
            ));
        }
        ...
        if !scp.success() {
            return Err(anyhow::anyhow!(
                ...
                local_path.display(), remote_target, scp.code()
            ));
        }
...
impl StorageRepository for LocalStorageRepository {
    async fn list_categories(&self) -> Result<Vec<String>> {
        ...
        // Local filesystem first; Oracle only as fallback when the path doesn't exist locally.
        ...
        self.list_cache.insert(cache_key, Arc::new(result.clone())).await;
        Ok(result)
    }
    ...
    async fn list_decks(&self, category: &str) -> Result<Vec<String>> {
        ...
        // Con SYNC_TO_ORACLE, Oracle es la fuente de verdad (lista todos los mazos reales).
        if self.sync_to_oracle && !self.oracle_host.is_empty() {
            ...
            if result.is_empty() {
                result = self.list_oracle_entries(&rel, false).await.unwrap_or_default();
            }
        }

        // Sin Oracle o si falló el listado remoto: disco local.
        if result.is_empty() {
    ...
    async fn get_deck_data(&self, category: &str, deck_name: &str) -> Result<DeckData> {
        ...
        // Con SYNC_TO_ORACLE: Oracle es la fuente de verdad para el JSON del mazo.
        let data = if self.sync_to_oracle {
            match self.fetch_from_oracle(&object_name).await {
                Ok(bytes) => bytes,
                Err(e) if path.exists() => {
                    tracing::warn!("Oracle falló para {} ({:?}), usando copia local", object_name, e);
                    fs::read(&path).await.context(format!("Failed to read {}", object_name))?
                }
                Err(e) => return Err(e.context(format!("Deck no encontrado en Oracle: {}", object_name))),
            }
        } else if path.exists() {
            fs::read(&path).await.context(format!("Failed to read {}", object_name))?
        } else {
        ...
        self.deck_cache.insert(cache_key, Arc::new(deck.clone())).await;
        Ok(deck)
    }
    ...
    async fn save_deck_data(&self, category: &str, deck_name: &str, data: &DeckData) -> Result<()> {
        ...
        self.list_cache.invalidate(&format!("decks:{}/{}", self.json_prefix, category)).await;
    ...
    async fn upload_blob(&self, blob_path: &str, content: Vec<u8>, _content_type: &str) -> Result<()> {
        let path = self.get_full_path(blob_path);
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent).await?;
        }
        ...
        if self.sync_to_oracle {
            ...
            // si falla, propagamos el error al endpoint y el cliente recibe un 500
            // claro en vez de una URL que apunta a un archivo inexistente (→ 404).
            self.scp_to_oracle(&path, blob_path).await
                .map_err(|e| {
        ...
        if self.sync_to_oracle && !self.oracle_host.is_empty() {
            ...
            match output {
                ...
                Ok(out) => {
                    ...
                    tracing::warn!("⚠️ blob_exists SSH falló para '{}': {}", blob_path, stderr.trim());
                }
                Err(e) => {
    ...
    async fn find_blob_by_prefix(&self, prefix: &str) -> Result<Option<String>> {
        ...
        let dir_str = prefix_path.parent().and_then(|p| p.to_str()).unwrap_or("").to_string();
        let file_prefix = prefix_path.file_name().and_then(|n| n.to_str()).unwrap_or("").to_string();

        // Oracle primero cuando SYNC_TO_ORACLE
        if self.sync_to_oracle {
