...
impl SurrealRepository {
    pub async fn new(endpoint: &str, namespace: &str, database: &str) -> Result<Self> {
        ...
        db.signin(Root {
            username: &user,
            password: &pass,
        }).await.map_err(|e| anyhow!("SurrealDB Auth Error: {}", e))?;

        db.use_ns(namespace).use_db(database).await?;
        tracing::info!("🚀 Conectado a SurrealDB (RocksDB) en {} (NS: {}, DB: {})", endpoint, namespace, database);
...
#[async_trait]
impl StoryArcadeRepository for SurrealRepository {
    async fn log_user_error(&self, user_id: &str, story_id: i32, screen_id: i32, user_input: &str, correct_answer: &str, error_type: &str, explanation: &str) -> Result<()> {
        ...
        let id = format!("user_errors:[{}, {}, {}]", user_id, story_id, now.timestamp_micros());
        
        self.db.query("UPDATE type::thing($id) MERGE {
            user_id: $user_id,
            story_id: $story_id,
            screen_id: $screen_id,
    ...
    async fn get_progress(&self, user_id: &str, story_id: i32) -> Result<Option<UserProgress>> {
        ...
        let mut res = self.db.query("SELECT * FROM type::thing($id)")
            .bind(("id", id))
            .await?;
    ...
    async fn create_progress(&self, user_id: &str, story_id: i32, episode_id: i32) -> Result<UserProgress> {
        ...
        let id = format!("user_progress:['{}', {}]", user_id, story_id);
        let mut res = self.db.query("UPDATE type::thing($id) CONTENT $data RETURN AFTER")
            .bind(("id", id))
            .bind(("data", progress))
            .await?;
    ...
    async fn reset_progress(&self, user_id: &str, story_id: i32) -> Result<()> {
        let id = format!("user_progress:['{}', {}]", user_id, story_id);
        self.db.query("DELETE type::thing($id)")
            .bind(("id", id))
            .await?;
        Ok(())
    }
    ...
    async fn get_story_title(&self, story_id: i32) -> Result<String> {
        let mut res = self.db.query("SELECT title FROM stories WHERE story_id = $id")
    ...
    async fn get_episode_title(&self, episode_id: i32) -> Result<String> {
        let mut res = self.db.query("SELECT title FROM episodes WHERE episode_id = $id")
            .bind(("id", episode_id)).await?;
        let title: Option<String> = res.take("title")?;
    ...
    async fn get_next_episode_id(&self, current_episode_id: i32) -> Result<Option<i32>> {
        ...
        ").bind(("id", current_episode_id)).await?;
        let id: Option<i32> = res.take("episode_id")?;
        Ok(id)
    }
    ...
    async fn update_screen_content(&self, screen_id: i32, content: serde_json::Value) -> Result<()> {
        ...
        self.db.query("UPDATE type::thing($id) MERGE { content: $content }")
            .bind(("id", id))
    ...
    async fn get_story_full_history(&self, story_id: i32) -> Result<serde_json::Value> {
        ...
        let mut scr_res = self.db.query("SELECT * FROM story_screens WHERE story_id = $id ORDER BY episode_id, step_order")
            .bind(("id", story_id)).await?;
    ...
    async fn get_episodes_by_story(&self, story_id: i32) -> Result<Vec<(i32, String)>> {
        ...
        #[derive(Deserialize)]
        struct EpRow { episode_id: i32, title: String }
...
impl CardProgressRepository for SurrealRepository {
    async fn upsert_card_progress(&self, user_id: &str, category: &str, deck: &str, card_index: i32, learned: bool) -> Result<()> {
        ...
        let id = format!("card_progress:['{}', '{}', '{}', {}]", user_id, category, deck, card_index);
...
impl UserRepository for SurrealRepository {
    async fn get_user_by_email(&self, email: &str) -> Result<Option<User>> {
        let mut res = self.db.query("SELECT * FROM user WHERE email = $email")
            .bind(("email", email))
            .await?;
    ...
    async fn upsert_user(&self, user: User) -> Result<User> {
        ...
        updated.map(|u| u.into()).ok_or_else(|| anyhow!("Failed to upsert user"))
    }

    async fn list_all_users(&self) -> Result<Vec<User>> {
        let mut res = self.db.query("SELECT * FROM user ORDER BY last_login DESC").await?;
...
impl SubscriptionRepository for SurrealRepository {
    async fn get_subscription(&self, email: &str) -> Result<Option<Subscription>> {
        let mut res = self.db.query("SELECT * FROM type::thing('subscription', $email)")
            .bind(("email", email))
            .await?;
