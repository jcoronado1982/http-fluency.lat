...
```mermaid
graph LR
    Dev[dev-* módulos] -->|merge| DevFull[dev-full]
    DevFull -->|PR / merge| QA_Branch[qa]
...
> Trabajo en **`dev-flashcards` / `dev-pronoun` / `dev-admin`**, integración en **`dev-full`**. Ver [GIT_BRANCHES.md](GIT_BRANCHES.md).
